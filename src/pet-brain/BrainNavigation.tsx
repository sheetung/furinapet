...
export function BrainNavigation() {
  ...
  async function refresh() {
    try {
      const next = await desktop.getAiSettings();
      setSettings(next);
      setForm({
      ...
      showToast(`AI 设置加载失败：${String(error)}`);
    }
  }
  ...
  return createPortal(
    <section className="page brain-page">
      ...
      <div className="brain-header">
        <span>Pet Brain</span>
        <h1>自主</h1>
        <p>本地 Utility Planner 仍然拥有最终决定权。AI 只接收抽象状态并给出高层 Goal 建议，不读取代码、Prompt、终端输出，也不能直接控制动画或坐标。</p>
      </div>
      ...
      <div className="brain-card">
        ...
        <div className="brain-note">即使关闭 AI 或 Provider 不可用，随机漫步替代、点击响应、Agent 生命周期和行为优先级仍由 Pet Brain 在本地正常工作。</div>
      </div>
      ...
      <div className="brain-card">
        <div className="brain-card-head">
          ...
          <span className={`brain-badge ${settings?.enabled ? "live" : ""}`}>{settings?.enabled ? "已启用" : "未启用"}</span>
        </div>
        ...
        <div className="brain-fields">
          ...
          <div className="brain-field">
            <label><strong>API Key</strong><small>{settings?.hasApiKey ? "已保存在 Windows Credential Manager；留空保持不变。" : "云端 Provider 通常需要；无密钥的本地模型可留空。"}</small></label>
            <input className="brain-input" type="password" autoComplete="off" value={form.apiKey} disabled={busy} placeholder={settings?.hasApiKey ? "已保存 · 输入新值可替换" : "可选"} onChange={(event) => setForm((current) => ({ ...current, apiKey: event.target.value }))} />
          </div>
          <div className="brain-field">
        ...
        <div className="brain-note">发送给 AI 的只有：当前 Goal、mood、energy、最近 Goal、Agent 分类状态、是否连接、用户多久未互动、点击 streak，以及是否允许漫步/停靠。</div>
        <div className="brain-actions">
          <span className="brain-badge">{settings?.configured ? "Provider 已配置" : "等待配置"}</span>
          <div>
            {settings?.hasApiKey && <button className="secondary brain-danger" disabled={busy || testing} onClick={() => void save(true)}>清除密钥</button>}
